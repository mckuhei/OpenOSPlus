...
if options.iname and options.name then
  io.stderr:write(i18n['find.cannotdefinebothinameandname'])
  return 1
end
...
if options.type then
  ...
  elseif options.type == "s" then
    ...
    io.stderr:write(string.format(i18n['find.unknownarg'], options.type))
    io.stderr:write(USAGE..'\n')
    return 1
  end
end
...
if options.iname or options.name then
  ...
  if type(fileNamePattern) ~= "string" then
    io.stderr:write(i18n['find.missingarg'])
    return 1
  end

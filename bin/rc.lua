...
local function load(name, args)
  ...
  if result then
    ...
    if result then
      ...
      return nil, string.format(i18n['rc.failedtostart'], fileName, reason)
    end
  end
  return nil, string.format(i18n['rc.failedtoload'], fileName, reason)
end
...
local function rawRunCommand(conf, name, cmd, args, ...)
  ...
  if result then
    if not cmd then
      io.output():write(string.format(i18n['rc.commandsforservice'], name))
    ...
    elseif cmd == "enable" then
      ...
      for _, _name in ipairs(conf.enabled) do
        if name == _name then
          return nil, i18n['rc.alreadyenabled']
        end
      end
    ...
    elseif cmd == "disable" then
      ...
      what = string.format(i18n['commandnotfound'], cmd, name)
    end
  end
  return nil, what
end
...
if select("#", ...) == 0 then
  ...
  if not results then
    local msg = string.format(i18n['rc.rcfailedtostart'], tostring(reason))
    write(stream, msg, "\n")
    return
  end

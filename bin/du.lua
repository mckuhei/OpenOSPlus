...
local TRY=i18n['du.tryhelp']

local VERSION=i18n['du.verison']


local HELP=i18n['du.usage']
...
if next(options) then
  for op,v in pairs(options) do
    io.stderr:write(string.format(i18n['du.invalid'], op))
  end
  io.stderr:write(TRY..'\n')
  return 1
end
...
for i,arg in ipairs(args) do
  ...
  if not fs.exists(path) then
    io.stderr:write(string.format("du: ".. i18n['cannotaccess'] .. "'%s': ".. i18n['nosuchfile'] .. "\n", arg))
    return 1
  else

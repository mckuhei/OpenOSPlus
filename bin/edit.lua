...
if fs.exists(file_parentpath) and not fs.isDirectory(file_parentpath) then
  io.stderr:write(string.format(i18n['edit.notadirectory'], file_parentpath))
  return 1
end
...
local keyBindHandlers = {
  ...
  save = function()
    ...
    if f then
      ...
      local format
      if new then
        format = i18n['edit.status.newWritten']
      else
        format = i18n['edit.status.written']
      end
...
do
  ...
  if f then
    ...
    table.insert(buffer, "")
    setStatus(string.format(i18n['edit.status.newfile'], fs.name(filename)))
  end
  setCursor(1, 1)
end
